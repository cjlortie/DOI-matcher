...
net.difference <- net.difference %>% select(Title, DOI)
net.difference
```

###Interpretation
1. Topological count returns from Web of Science at least to do not necessarily estimate extent of differences.  
2. Using this workflow or approach ensures a more transparent repeat search and matching process (siumilar to the PRISMA goals for single search events).  
3. There is likely sensitivity and bias in this process that use of R can help mitigate.